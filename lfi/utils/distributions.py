...
class copula:
    ...
    def cdf_per_dim(X):
            ...
            dim = X.shape[1]
            for k in range(dim):
                quantiles, CDF = umath.ecdf(np.asarray(X[:, k]))
    ...
    def X2U(X, cdf_per_dimension):
            ...
            for i in range(n):
                ...
                for k in range(dim):
                    ...
                    u.append(umath.get_ecdf_val(quantile, cdf, x[0, k]))
                U.append(np.array(u))
    ...
    def X2Z(X, cdf_per_dimension):
            ...
            for i in range(n):
                ...
                for k in range(dim):
                    quantile = cdf_per_dimension[k][0]
                    cdf = cdf_per_dimension[k][1]
                    z.append(stats.norm.ppf(umath.get_ecdf_val(quantile, cdf, x[0, k])))
                Z.append(np.array(z))
    ...
    def Z2X(Z, cdf_per_dimension):
            ...
            for i in range(n):
                ...
                for k in range(dim):
                    quantile = cdf_per_dimension[k][0]
                    cdf = cdf_per_dimension[k][1]
                    x.append(umath.inverse_ecdf(quantile, cdf, stats.norm.cdf(z[k])))
                X.append(np.array(x))
            return np.array(X)
